...
> **核心洞察**：DTC的8位Status实现了一个**全自动的Debounce(消抖)+确认+老化**的状态机，而不是随机决定要不要点亮MIL灯的参数。一次颠簸导致传感器连接松动不一定是真故障，pendingDTC提供了"先观察、别急着存档"的过渡态。连续两个行驶循环才确认，这是"下次再看看"的设计，是保守且正确的。
...
> **【下集预告】**：症状记下来了，治好了怎么办？得清除DTC。0x14 ClearDiagnosticInformation。要一口气清除整组DTC、相关的快照数据、冻结帧和扩展记录，而不是只删除某一个DTC。但是！镜像内存的记录不会被清除，这是法律留下的永久痕迹。
